function generateDom(text_template, extend){
	var stack = [];
	var collect = "";
	var type = "";
	var value = "";
	...
	for(var i = 0; i < text.length; i++){
		if((text[i] != "{") && (text[i] != "}") && (text[i] != ";") && (text[i] != ":") || ((quote != "") && ((text[i] == "{") || (text[i] == "}")))){
			...
			if((blocks[blocks.length - 1].cC == "") & (code[code.length - 1] != ">")){
				code += '>';
			}
			code += blocks[blocks.length - 1].cC;
			...
			if(quote == ""){
				...
				}else if(type.split(/\s/).join("") in extend){
					var tmpExtend = extend[type.split(/\s/).join("")](value);
					blocks[blocks.length - 1].cC += tmpExtend.cC;
					blocks[blocks.length - 1].cP += tmpExtend.cP;
					blocks[blocks.length - 1].cS += tmpExtend.cS;
				}else{
